function [F,M] = FM_BB2_6DOF_10th_Feb(vel,rates,omega,param)
% This model is identified from DRF + SRF+ Nominal data
% input:
% vel   : velocity projection on the body frame, m/s
% rates : angular velocities in rad/s
% omega: rotor speeds in rad/s
% ouput:
% F\M: aerodynamic forces and moments on the body frame, N \ Nm
%
% Sihao Sun 10th-Feb-2019
% sihao.sun@outlook.com

persistent aero_model;

if isempty(aero_model)
    aero_model = load('model_individual_damage_BB2_simple_10th_Feb.mat');
end
...
if va>=0.01
    ...
    alpha = 0; u_bar = 0; v_bar = 0; w_bar = 0;
end
...
T0 = -P32(sign(w_bar).*abs(w_bar),abs(v_bar),va.^2*S*1.225/2.*sign(w_bar))*aero_model.k_model{1};
...
T1 = (Ct01)*dynhead1*Area * d_ratio(1);
T2 = (Ct02)*dynhead2*Area * d_ratio(2);
T3 = (Ct03)*dynhead3*Area * d_ratio(3);
T4 = (Ct04)*dynhead4*Area * d_ratio(4);

X1 = (u1*omega1*aero_model.k_model{4} + SN(1)*v1*omega1*aero_model.k_model{5})  * d_ratio(1);
X2 = (u2*omega2*aero_model.k_model{4} + SN(2)*v2*omega2*aero_model.k_model{5})  * d_ratio(2);
X3 = (u3*omega3*aero_model.k_model{4} + SN(3)*v3*omega3*aero_model.k_model{5})  * d_ratio(3);
X4 = (u4*omega4*aero_model.k_model{4} + SN(4)*v4*omega4*aero_model.k_model{5})  * d_ratio(4);

Y1 = (-SN(1)*u1*omega1*aero_model.k_model{5} + v1*omega1*aero_model.k_model{4})  * d_ratio(1);
Y2 = (-SN(2)*u2*omega2*aero_model.k_model{5} + v2*omega2*aero_model.k_model{4})  * d_ratio(2);
Y3 = (-SN(3)*u3*omega3*aero_model.k_model{5} + v3*omega3*aero_model.k_model{4})  * d_ratio(3);
Y4 = (-SN(4)*u4*omega4*aero_model.k_model{5} + v4*omega4*aero_model.k_model{4})  * d_ratio(4);

L1 = (SN(1)*u1*omega1*aero_model.k_model{7} - v1*omega1*aero_model.k_model{6})  * d_ratio(1);
L2 = (SN(2)*u2*omega2*aero_model.k_model{7} - v2*omega2*aero_model.k_model{6})  * d_ratio(2);
L3 = (SN(3)*u3*omega3*aero_model.k_model{7} - v3*omega3*aero_model.k_model{6})  * d_ratio(3);
L4 = (SN(4)*u4*omega4*aero_model.k_model{7} - v4*omega4*aero_model.k_model{6})  * d_ratio(4);

M1 = (u1*omega1*aero_model.k_model{6} + SN(1)*v1*omega1*aero_model.k_model{7}) * d_ratio(1);
M2 = (u2*omega2*aero_model.k_model{6} + SN(2)*v2*omega2*aero_model.k_model{7}) * d_ratio(2);
M3 = (u3*omega3*aero_model.k_model{6} + SN(3)*v3*omega3*aero_model.k_model{7}) * d_ratio(3);
M4 = (u4*omega4*aero_model.k_model{6} + SN(4)*v4*omega4*aero_model.k_model{7}) * d_ratio(4);

N1 = SN(1)*(Cq01)*dynhead1*Area * d_ratio(1);
N2 = SN(2)*(Cq02)*dynhead2*Area * d_ratio(2);
N3 = SN(3)*(Cq03)*dynhead3*Area * d_ratio(3);
N4 = SN(4)*(Cq04)*dynhead4*Area * d_ratio(4);
